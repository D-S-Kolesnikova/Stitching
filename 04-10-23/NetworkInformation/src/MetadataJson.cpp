...
ItvCv::PointMatchingParams MetadataJson<NodeType::PointMatchingParameters>::Parse(
    const Json::Value& networkParametersNode)
{
    ItvCv::PointMatchingParams result;
    const auto& PointMatchingNode = GetRequiredNodeByTag(
        networkParametersNode,
        Tags::JSON_POINT_MATCHING);

    result.maxNumKeypoints = GetRequiredNodeByTag(PointMatchingNode, Tags::JSON_NUM_KEYPOINTS).asInt();
    result.numDescriptors = GetRequiredNodeByTag(PointMatchingNode, Tags::JSON_NUM_DESCRIPTORS).asInt();
    return result;
}

Json::Value MetadataJson<NodeType::PointMatchingParameters>::Dump(const ItvCv::PointMatchingParams& pointMatchingParams)
{
    Json::Value PointMatchingNode;
    const auto pointMatchingData = boost::get<ItvCv::PointMatchingParams>(pointMatchingParams);
    PointMatchingNode[Tags::JSON_POSITION] = pointMatchingData.maxNumKeypoints;

    return PointMatchingNode;
}
    ...
    const ItvCv::CommonParams& commonParams)
{
    ...
    case itvcvAnalyzerPointMatcher:
        return MetadataJson<NodeType::PointMatchingParameters>::Parse(networkParametersNode);
    default:
        throw std::runtime_error(
