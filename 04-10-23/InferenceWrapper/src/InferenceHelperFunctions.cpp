...
constexpr auto POINT_MATCHER_OUTPUT_KPTS = "0keypoints";
constexpr auto POINT_MATCHER_OUTPUT_DESC = "0descriptors";
constexpr auto POINT_MATCHER_OUTPUT_MATCHES = "matches";
constexpr auto POINT_MATCHER_OUTPUT_SCORES = "mscores";
}
...
template<>
std::vector<AnalyzerTraits<itvcvAnalyzerPointMatcher>::ResultType> ProcessIntelIEOutput<itvcvAnalyzerPointMatcher>(
    ov::InferRequest inferRequest,
    const std::size_t batchSize,
    const PreprocessContext& pp,
    const ItvCv::PNetworkInformation& networkInformation)
...
using PointMatchingData_t = InferenceWrapper::AnalyzerTraits<itvcvAnalyzerPointMatcher>::ResultType;

template<>
std::vector<AnalyzerTraits<itvcvAnalyzerPointMatcher>::ResultType> ProcessTensorRTOutput<itvcvAnalyzerPointMatcher>(
    const CBufferManager& bufMgr,
    const int& batchSize,
    const std::string& outputLayerName,
    const PreprocessContext& pp,
    const ItvCv::PNetworkInformation& networkInformation)
{
    PointMatchingData_t featureOutputData;
    ...
    for (const auto& nodeName : names)
    {
        ...
        if (boost::algorithm::contains(nodeName, POINT_MATCHER_OUTPUT_KPTS))
        {
            uint64_t id =(*(nodeName.end() - 1)) - '0';
            featureOutputData.keypoints[id] = { {dataPtr, dataPtr + elementSize}, dimsArray };
        }
        else if (boost::algorithm::contains(nodeName, POINT_MATCHER_OUTPUT_DESC))
        {
            uint64_t id = (*(nodeName.end() - 1)) - '0';
            featureOutputData.descriptors[id] = { {dataPtr, dataPtr + elementSize}, dimsArray };
        }
        else if (boost::algorithm::contains(nodeName, POINT_MATCHER_OUTPUT_MATCHES))
        {
            uint64_t id = (*(nodeName.end() - 1)) - '0';
            featureOutputData.matches[id] = { {dataPtr, dataPtr + elementSize}, dimsArray };
        }
        else if (boost::algorithm::contains(nodeName, POINT_MATCHER_OUTPUT_SCORES))
        {
            uint64_t id = (*(nodeName.end() - 1)) - '0';
            featureOutputData.scores[id] = { {dataPtr, dataPtr + elementSize}, dimsArray };
        }
    }
    std::vector<PointMatchingData_t> results;
...
template<>
std::vector<AnalyzerTraits<itvcvAnalyzerPointMatcher>::ResultType> ProcessAtlas300Output<itvcvAnalyzerPointMatcher>(
    const ResultData& res,
    const PreprocessContext&,
    ItvCv::PNetworkInformation const&)
{
