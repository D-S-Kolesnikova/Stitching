...
template class Channel<itvcvAnalyzerPointMatcher>;
...
std::unique_ptr<IInferenceEngine<itvcvAnalyzerMaskSegments>> IInferenceEngine<itvcvAnalyzerMaskSegments>::Create(
    ...
    const InferenceChannelParams& channelParams);
template
std::unique_ptr<IInferenceEngine<itvcvAnalyzerPointMatcher>> IInferenceEngine<itvcvAnalyzerPointMatcher>::Create(
    itvcvError& error,
    const EngineCreationParams& engineParams,
    const InferenceChannelParams& channelParams);

template
typename InferenceResultTraits<itvcvAnalyzerPointMatcher>::AsyncInferenceResult_t Channel<itvcvAnalyzerPointMatcher>::AsyncProcessFrame(
    const std::vector<ItvCv::Frame>& frames);
