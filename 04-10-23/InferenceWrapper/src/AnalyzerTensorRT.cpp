...
public:
    ...
    typename InferenceResultTraits<analyzerType>::AsyncInferenceResult_t AsyncInfer(const std::vector<ItvCv::Frame>& frames, InferenceCounter) override;
...
#ifndef __aarch64__
    void PreprocessGpuFrame(const ItvCv::Frame& frame, void* inputBuffer, PreprocessContext& ctx, cudaStream_t stream, const int batchInd = 0)
    {
        ...
        if (m_engineParameters->net->inputParams.pixelFormat == ItvCv::PixelFormat::GRAY)
        {
            cv::cuda::cvtColor(tmpFrame, tmpFrame, cv::COLOR_BGR2GRAY);
        }

        auto inputChannels = WrapOneSampleInBuffer<cv::cuda::GpuMat>(inputBuffer, batchInd);
    ...
    const std::vector<ItvCv::Frame>& frames, InferenceCounter counter)
{
    ...
    for(auto i =0; i < frames.size(); i++)
    {
        ...
        {
            PreprocessGpuFrame(frames[i], request->bufferManager.GetDeviceBuffer(m_trtEngineParameters.inputTensorNames[0]), ctx, request->stream, i);
        }
        else
#endif
...
bool AnalyzerTensorRT<analyzerType>::Build()
{
    ...
    if (m_engine)
    {
        ...
        m_trtEngineParameters.batchSize = inputDims.d[0];
    }
    else
    {
        ...
        if (m_inputGeometry.area())
        {
            ...
            {
                network->getInput(0)->setDimensions(nvinfer1::Dims4(
                    m_trtEngineParameters.batchSize,
                    m_engineParameters->net->inputParams.numChannels,
    ...
    if (
        ...
        || analyzerType == itvcvAnalyzerType::itvcvAnalyzerPointMatcher)
    {
...
std::unique_ptr<IAnalyzer<itvcvAnalyzerSiamese>> CreateTensorRTAnalyzer<itvcvAnalyzerSiamese>(
        ...
        std::shared_ptr<EngineCreationParams> parameters);
template
std::unique_ptr<IAnalyzer<itvcvAnalyzerPointMatcher>> CreateTensorRTAnalyzer<itvcvAnalyzerPointMatcher>(
        itvcvError& error,
        std::shared_ptr<EngineCreationParams> parameters);

template
typename InferenceResultTraits<itvcvAnalyzerPointMatcher>::AsyncInferenceResult_t AnalyzerTensorRT<itvcvAnalyzerPointMatcher>::AsyncInfer(
