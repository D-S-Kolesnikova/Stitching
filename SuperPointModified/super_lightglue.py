...
class SuperPointForLightglue(nn.Module):
    default_conf = {
        "descriptor_dim": 256,
        "nms_radius": 4,
    ...
    def forward(self, image: torch.Tensor):
        ...
        for i in range(b):
            ...
            k = normalize_keypoints(k, image.shape[2], image.shape[3])
            
            extendedKeypoints = torch.full((self.default_conf["max_num_keypoints"], k.shape[1]), -1.0)
            extendedKeypoints[:k.shape[0], :k.shape[1]] = k
            k = extendedKeypoints
            
            keypoints.append(k[None])
            descriptors.append(d.transpose(1, 2))

        return {
            "keypoints0": keypoints[0],
            "keypoints1": keypoints[1],
            "descriptors0": descriptors[0],
