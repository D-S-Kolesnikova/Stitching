...
class LightGlue(nn.Module):
    ...
    features = {
        "superpoint": ("superpoint_lightglue", 256, False),
        "disk": ("disk_lightglue", 128, False),
        "sift" : ("sift_lightglue", 128, True),
    }
    ...
    def __init__(self, features="superpoint", **conf) -> None:
        ...
        if features is not None:
            ...
            self.conf["weights"], self.conf["input_dim"], self.conf["add_scale_ori"] = self.features[features]
        self.conf = conf = SimpleNamespace(**self.conf)

        if conf.input_dim != conf.descriptor_dim:
            self.input_proj = nn.Linear(conf.input_dim, conf.descriptor_dim, bias=True, device="cuda")
        else:
    ...
    def forward(
        ...
        # kpts0 = normalize_keypoints(kpts0, imsize0).clone()
        # kpts1 = normalize_keypoints(kpts1, imsize1).clone()
        ...
        mask = torch.all(kpts0 != -1.0, dim=2)
        kpts0 =  kpts0[mask].unsqueeze(0)
        mask = torch.all(kpts1 != -1.0, dim=2)
        kpts1 =  kpts1[mask].unsqueeze(0)
        desc0 = desc0[:, 0:kpts0.shape[1], :]
        desc1 = desc1[:, 0:kpts1.shape[1], :]
        data0["scales"] = data0["scales"][:, 0:kpts0.shape[1]]
        data1["scales"] = data1["scales"][:, 0:kpts1.shape[1]]
        data0["oris"] = data0["oris"][:, 0:kpts0.shape[1]]
        data1["oris"] = data1["oris"][:, 0:kpts1.shape[1]]
        ...
        for i in range(self.conf.n_layers):
            ...
            if do_point_pruning and desc0.shape[-2] > pruning_th:  # point pruning
        ...
        extendedMatches = torch.full((2048, 2), -1).cuda()
        extendedMatches[:matches.shape[0], :matches.shape[1]] = matches
        matches = extendedMatches
        extendedScores = torch.full(torch.Size([2048]), -1).cuda()
        extendedScores[:mscores.shape[0]] = mscores
        mscores = extendedScores
        matches = matches.to(device, dtype=torch.float32)
        mscores = mscores.to(device, dtype=torch.float32)
        
        # extendedm0 = torch.full((1, 2048), -1, dtype=torch.float32).cuda()
        # extendedm0[:m0.shape[0], :m0.shape[1]] = m0
        # m0 = extendedm0
        # extendedm1 = torch.full((1, 2048), -1, dtype=torch.float32).cuda()
        # extendedm1[:m1.shape[0], :m1.shape[1]] = m1
        # m1 = extendedm1
        # extendedmscores0 = torch.full((1, 2048),  0.0, dtype=torch.float32).cuda()
        # extendedmscores0[:mscores0.shape[0], :mscores0.shape[1]] = mscores0
        # mscores0 = extendedmscores0
        # extendedmscores1 = torch.full((1, 2048),  0.0, dtype=torch.float32).cuda()
        # extendedmscores1[:mscores1.shape[0], :mscores1.shape[1]] = mscores1
        # mscores1 = extendedmscores1
        # return m0, m1, mscores0, mscores1
