...
typedef boostMpl::map<
    ...
    boostMpl::pair<boostMpl::int_<static_cast<int>(itvcvAnalyzerType::itvcvAnalyzerPointDetection)>, ItvCv::PointDetectionParams>,
...
ITVCV_NETINFO_API void Validation(const ItvCv::NetworkInformation& data)
{
    ...
    case itvcvAnalyzerPointDetection:
        ValidationNetParameters<itvcvAnalyzerPointDetection>(data);
    default:
...
ITVCV_NETINFO_API void Validation(const ItvCv::PointDetectionParams& data)
{
    if (data.maxNumKeypoints == 0)
    {
        throw ValidationException(
            "maxNumKeypoints is empty",
            ValidationError::BadPointDetectionParams);
    }
}
