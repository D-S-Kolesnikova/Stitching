...
void writeToFile(const int nFrames, const vector<CameraParams>& cameras, const std::vector<Mat>& input_img);
...
private:
    ...
    string superPointPath = "E:/devel/helper/homography/projects/LightGlue-ONNX/weights/superpoint.onnx";
    string lightGluePath = "E:/devel/helper/homography/projects/LightGlue-ONNX/weights/superpoint_lightglue.onnx";
public:
        ...
        , m_colorSchemes(colorSchemes)
    {
        *outputError = ESTNoError;
        // Осуществим проверку корректности параметров
        ...
        {
            matcher = makePtr<LightGlue>(lh, Stitcher::PANORAMA, match_conf);
        }
        else
        {
        ...
        writeToFile(nFrames, cameras, input_img);
...
IStitchingWrapper* CreateStitchingEngine(const int nFrames, const int direction, const int* widths, const int* heights, const int* strides, const int* colorSchemes,
                                         const unsigned char* frameDataBGR, float* outputIntinsicCameraMatrix, float* outputRotationMatrix, float* ratio, stError* outputError)
{
    return new CStitchingWrapper(nFrames, direction, widths, heights, strides, colorSchemes, frameDataBGR, outputIntinsicCameraMatrix, outputRotationMatrix, ratio, outputError);
}
